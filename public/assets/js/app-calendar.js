...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    function eventClick(info) {
      ...
      if (offcanvasTitle) {
        offcanvasTitle.innerHTML = 'Actualizar plazo';
      }

      const event = eventToUpdate.extendedProps;

      $('#title-add').val(event.title);
      $('#tipo-add').val(event.type.id).trigger('change');
      $('#marca-add').val(event.brand.id).trigger('change');
      $('#pais-add').val(event.country).trigger('change');
      const clases = event.class.reduce((acc, clase) => {
        acc.push(clase.id);
        return acc;
      }, [])
      $('#clasesNiza-Add').val(clases).trigger('change');

      const reminders = event.reminders.reduce((acc, reminder) => {
        acc.push(reminder.id);
        return acc;
      }, [])
      $('#recordatorios-Add').val(reminders).trigger('change');
      $('#responsable-add').val(event.responsability).trigger('change');
      $('#eventEndDate').val(event.limit).trigger('change');

      event.channels.map(channel => {
        $(`#notification-${channel.id}`).attr('checked', true)
      })
    ...
    function selectedCalendars() {
      ...
      filterInputs.forEach(item => {
        if (item.type === 'checkbox' && item.checked) {
          ...
          // Para <select multiple> o <select normal>
          Array.from(item.selectedOptions).forEach(opt => {
    ...
    async function fetchEvents(info, successCallback) {
      ...
      const {data: alerts} = await fetchHelper.get(base_url(['dashboard/alertboard/data'], {
    ...
    let calendar = new Calendar(calendarEl, {
      ...
      dateClick: function (info) {
        ...
        if (offcanvasTitle) {
          offcanvasTitle.innerHTML = 'Añadir plazo';
        }
    ...
    if (select2.length) {
      ...
      $('#eventForm .form-check-input').attr('checked', false);
      // eventEndDate.value = '';
      // eventUrl.value = '';
    ...
    if (filterInput) {
      ...
      filterInput.forEach(item => {
        item.addEventListener('change', () => {
          // Comprobamos si todos los filtros están activos/seleccionados
          const allChecked = filterInput.every(c => {
            if (c.type === 'checkbox') return c.checked;
            if (c.tagName === 'SELECT') return c.selectedOptions.length > 0;
            if (c.type === 'text') return c.value.trim() !== '';
            return true;
          });
      
          // Actualizamos el estado del checkbox "Select All"
          selectAll.checked = allChecked;
      
          // Refetch del calendario
          calendar.refetchEvents();
        });
      });
    }
